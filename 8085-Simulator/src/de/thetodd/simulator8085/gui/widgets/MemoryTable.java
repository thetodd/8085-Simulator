...
public class MemoryTable extends TableViewer implements ISimulatorListener {

	public MemoryTable(Composite parent) {
		super(parent, SWT.BORDER | SWT.FULL_SELECTION);
		getTable().setFont(
				SWTResourceManager.getFont("Courier New", 9, SWT.NORMAL));
	...
	public void globalSimulatorEvent(SimulatorEvent evt) {
		if (evt.getEvent().equals(GlobalSimulatorEvents.MEMORY_CHANGE)) {
			getTable().removeAll();
			for (int i = Memory.getInstance().getMemoryStart(); i <= Memory
				...
				for (int j = 0; j < 16; j++) {
					...
					mem[1 + j] = String.format("%02x", Memory.getInstance()
							.get((short) (i + j)));
				}
