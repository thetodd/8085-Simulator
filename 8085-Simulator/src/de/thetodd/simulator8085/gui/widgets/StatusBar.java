...
public class StatusBar extends Composite implements ISimulatorListener {
	...
	public void globalSimulatorEvent(SimulatorEvent evt) {
		switch (evt.getType()) {
		case ERROR:
			lblStatus.setForeground(new Color(Display.getDefault(), 255, 0, 0));
			lblStatus.setImage(ERROR_IMG);
			break;
		case INFORMATION:
			lblStatus
					.setForeground(new Color(Display.getDefault(), 0, 50, 200));
			lblStatus.setImage(INFORMATION_IMG);
			break;
		case SUCCESS:
			lblStatus.setForeground(new Color(Display.getDefault(), 0, 150, 0));
			lblStatus.setImage(SUCCESS_IMG);
			break;
		case WARNING:
			lblStatus
					.setForeground(new Color(Display.getDefault(), 150, 150, 0));
